...
df2025 = df2025[df2025["fantasy_points"].notna()]
...
total_fantasy_points = combined[(combined["season"] == 2025) & (combined["week"] <= 8)] \
    .groupby("player_id")["fantasy_points"].sum()

# Keep only players with >0 passing yards
active_players = total_fantasy_points[total_fantasy_points > 0].index
...
last_week["predicted_fantasy_points"] = y_pred
...
print(last_week[["player_id", "name", "predicted_fantasy_points"]].head(50))
last_week[["player_id", "name", "predicted_fantasy_points"]].to_csv("predictions/week8_fantasy_predictions.csv", index=False)
